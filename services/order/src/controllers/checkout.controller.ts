...
export const checkout: any = async (req: Request, res: Response, next: NextFunction) => {
    try {
    ...
    const productDetails = await Promise.all(
        cartItem.data.map(async (item) => {
            const {data: productData} = await axios.get(`${PRODUCT_SERVICE}/products/${item.productId}`);
            return {
    ...
    await axios.post(`${EMAIL_SERVICE}/emails/send`,{
        ...
        source: "checkout",
        message: `Thank you for your order. Your order number is ${order.id}. your otder total is ${grandTotal}`
    })
